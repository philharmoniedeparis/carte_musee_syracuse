...
/*     const layersToRemove = [
    ...
    toggleNoClusterLayers('remove', layersToRemove); */
    const layersToRemove = [
      ...Object.values(window.groups.countriesGroup),
...
const toggleNoClusterLayers = (action, layerCollection) => {

  Object.keys(layerCollection).forEach(key => {
    if (action === 'add' && !window.mapMusee.hasLayer(layerCollection[key])) {
        window.mapMusee.addLayer(layerCollection[key]);
    } else if (action === 'remove' && window.mapMusee.hasLayer(layerCollection[key])) {
...
function displayCountryMarkers(e) {
  ...
  var neighborsCode = getNeighbor(selectedLayer)
  ...
  var countriesToExclude = [/* ...neighborsCode,  */selectedCountry]
  var notNeighborsCode = continentCountriesList.filter(code => !countriesToExclude.includes(code));
  var notNeighborsLayers = [];

  notNeighborsCode.forEach(code => {
      // Utilisez filter pour obtenir les layers correspondants au code
      var filteredLayers = window.groups.continentGroup[continent].getLayers().filter(layer => (layer.data.country_code === code));
      
      // Ajoutez les layers filtrés à notNeighborsLayers
      notNeighborsLayers.push(...filteredLayers);
  ...
  const layersToAdd = [
    ...
    //...neighborsCode.map(code => window.groups.countriesGroup[code]),
    //...neighborsCode.map(code => window.unknown_markers.countries[code]),
    ...notNeighborsLayers,
    ...continentToAdd
  ...
  //toggleNoClusterLayers()

  
  // Retirer les layers inutiles
  /* var layersToRemove = [
    window.continents_popups, 
    window.groups.continentGroup, 
    window.unknown_markers.continents,
    window.countryMarkers,
    window.markers
  ] */
  //removeLayers(window.continents_popups)

  //[...window.continents_popups].filter(Boolean).forEach(removeLayers);

  // Ajouter les layers nécessaires
 /*  var layersToAdd = [
    window.groups.countriesGroup, 
    window.unknown_markers.countries
  ]
  layersToAdd.filter(Boolean).forEach(addLayers); */
...
function handleFilters() {
  ...
  $(document).on('mousedown', '.selectize-dropdown-content [data-value]', function(e) {
    ...
    if (!continentSelected) {
      Object.keys(window.topo_countries_layers[0]._layers).forEach(key => {
        var layer = window.topo_countries_layers[0]._layers[key];
...
const filterSearch = () => {
  ...
  // Data filter method
  const filterIt = (arr, query) => {
    return arr.filter(obj => Object.keys(obj).some(key => key !== 'Enregistrement' && new RegExp(query, "i").test(obj[key])));
  };
...
/** Fonction permettant de récupérer les pays voisins 
* @param {Object} countryLayer - Objet des informations topojson du pays sélectionné
**/
const getNeighbor = countryLayer => { 
  if (countryLayer.feature.properties.borders_iso_a2 === null) { return [] }

  const iso_borders_iso_a2 = countryLayer.feature.properties.borders_iso_a2.split(",");
  const neighborsLayers = [];
  const allLayers = window.topo_countries_layers[0]

  iso_borders_iso_a2.forEach(neighbor => {
    allLayers.eachLayer(layer => {
      // Vérifiez si layer.layerID existe avant de comparer
      if (layer.layerID && layer.layerID === neighbor) {
        neighborsLayers.push(layer);
      }
    });
  });


  return iso_borders_iso_a2;
};
